...
describe 'DELETE /lists/:id', type: :request do
  subject { response }
  ...
  context 'with valid request' do
    ...
    it { expect(project.lists.find_by(name: 'Foo')).to be_nil }
  end
    ...
    let(:error) do
      {
        ...
        message: "Invalid request.\n\n#: failed schema " \
          '#/definitions/list/links/3/schema: "foo" is not a ' \
          'permitted key.'
      }
    end

    before do
      delete "/lists/#{list.id}", params: { foo: 'bar' }.to_json
    end

    it { is_expected.to have_http_status(:bad_request) }
    it { expect(response.body).to eq_json(error) }
    it { expect(project.lists.find_by(name: 'Foo')).to eq(list) }
  end
  ...
  context 'without current user' do
    ...
    before do
      delete "/lists/#{list.id}", headers: headers
    end
  ...
  context 'when list is from another organization' do
  ...
  context 'when list is from another user from the same organization' do
